...
@app.route('/static_address')
def static_address():
    return render_template('static_address.html')

@app.route('/save_static_address', methods = ['POST'])
def save_static_address():
    interface = request.form['interface']
    ip_address = request.form['ip_address']
    routers = request.form['routers']
    domain_name_servers = request.form['domain_name_servers']
    write_static_address_config(interface, ip_address, routers, domain_name_servers)
    if interface == 'wlan0':
        wifi_ap_array = scan_wifi_networks()
        config_hash = config_file_hash()
        return render_template('app.html', wifi_ap_array = wifi_ap_array, config_hash = config_hash)
        ...
        t.start()
        return render_template('save_wired_static_address.html')
...
def write_static_address_config(interface, ip_address, routers, domain_name_servers):
    with open('/etc/dhcpcd.conf.original', 'a+') as saconf:
        saconf.write("interface %s\n" % interface)
        saconf.write("static ip_address=%s\n" % ip_address)
        saconf.write("static routers=%s\n" % routers)
        saconf.write("static domain_name_servers=%s\n" % domain_name_servers)
